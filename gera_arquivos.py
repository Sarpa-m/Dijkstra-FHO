...
class App:
    ...
    def carregar(self):
        ...
        with open("nomes_estacoes.txt","r") as f:
            for ln in f:
                num,name=ln.strip().split('-',1); self.station_names[int(num)] = name
        self.N = len(grafo); self.entry_n.delete(0,tk.END); self.entry_n.insert(0,str(self.N))
    ...
    def adicionar(self):
        ...
        for idx,(u0,v0,_,_) in enumerate(self.connections):
            if u0==u and v0==v:
                ...
                break
        if not updated:
            self.connections.append((u,v,d,code)); self.tree.insert("","end",values=(u,v,d,line))
        self.update_preview()
    ...
    def adicionar_estacao(self):
        ...
        self.atualizar_spinboxes(); messagebox.showinfo("Info",f"Estação {self.N} adicionada"); self.update_preview()
    ...
    def remover_estacao(self):
        ...
        # remova conexões associadas e renumere restantes
        self.connections=[(x,y,d,c) for x,y,d,c in self.connections if x!=u and y!=u]
        # shift connections >u down by 1
        shifted=[]
        ...
        # renumera nomes keys >u
        names={i-1 if i>u else i:n for i,n in self.station_names.items()}
        ...
        messagebox.showinfo("Info",f"Estação {u} removida"); self.update_preview()

    def atualizar_spinboxes(self):
        self.spin_a.config(to=self.N); self.spin_b.config(to=self.N)

    def update_preview(self):
        self.ax.clear(); G=nx.Graph(); G.add_nodes_from(range(1,self.N+1))
        for u,v,d,c in self.connections:
            G.add_edge(u,v,weight=d,linha=c)
        pos=nx.spring_layout(G,seed=42)
        nx.draw_networkx_nodes(G,pos,ax=self.ax,node_color='lightgray',node_size=300)
        labels={i:self.station_names.get(i,f"E{i}") for i in G.nodes()}
        nx.draw_networkx_labels(G,pos,labels,ax=self.ax)
        for u,v,d in G.edges(data=True):
            nx.draw_networkx_edges(G,pos,[(u,v)],edge_color=code_to_color.get(d['linha'],'black'),ax=self.ax)
        self.ax.set_title('Preview do Grafo'); self.ax.axis('off'); self.canvas.draw()
    ...
    def salvar(self):
        ...
        for u,v,d,c in self.connections:
            ...
            dist[u-1,v-1]=dist[v-1,u-1]=d; lines[u-1,v-1]=lines[v-1,u-1]=c
        with open("estacoes.txt","w") as f:
            for i in range(1,self.N+1): f.write(f"{i}-{' '.join(map(str,sorted(ests[i])))}\n")
        np.savetxt("distancias.txt",dist,fmt="%.1f"); np.savetxt("linhas.txt",lines,fmt="%d")
        with open("nomes_estacoes.txt","w") as f:
            for i in range(1,self.N+1): f.write(f"{i}-{self.station_names.get(i,f'E{i}')}\n")
